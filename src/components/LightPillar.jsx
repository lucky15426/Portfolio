...
const LightPillar = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-full absolute top-0 left-0 flex items-center justify-center ${className}`}
      >
        <div
          style={{
            background: `radial-gradient(circle, ${bottomColor} 0%, ${topColor} 70%)`,
            opacity: 0.35,
            filter: 'blur(80px)',
            borderRadius: '50%',
            width: '320px',
            height: '320px',
            animation: 'pulseFallback 6s ease-in-out infinite',
          }}
        />
        <style>{`
