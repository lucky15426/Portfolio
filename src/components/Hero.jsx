...
function Hero() {
  ...
  const blob1Y = useTransform(mouseY, [-500, 500], [-30, 30]);
  const blob2X = useTransform(mouseX, [-500, 500], [40, -40]);
  const blob2Y = useTransform(mouseY, [-500, 500], [20, -20]);

  // Smooth springs
  const springConfig = { damping: 30, stiffness: 200 };
  const smoothBlob1X = useSpring(blob1X, springConfig);
  const smoothBlob1Y = useSpring(blob1Y, springConfig);
  const smoothBlob2X = useSpring(blob2X, springConfig);
  const smoothBlob2Y = useSpring(blob2Y, springConfig);
  ...
  const nameLetters = useMemo(() => "Lucky Kumar Gupta".split(""), []);

  return (
    <section
      id="home"
      className={`min-h-screen flex items-center justify-center px-6 py-20 relative overflow-hidden ${theme === "dark" ? "bg-[#1c1c1c]" : "bg-[#fafafa]"
        }`}
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <motion.div
          ...
          className={`absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full blur-3xl opacity-[0.15] ${theme === "dark" ? "bg-[#b8f2e6]" : "bg-[#aed9e0]"
            }`}
        />
      </div>
      ...
      >
        ...
        <motion.div
          ...
          className={`text-base md:text-lg mb-6 font-medium tracking-wide ${theme === "dark" ? "text-[#aed9e0]/70" : "text-[#5e6472]/60"
            }`}
        >
          Hello! I'm
        </motion.div>
        ...
        <motion.p
          ...
          className={`text-xl md:text-2xl lg:text-3xl mb-10 font-light leading-relaxed ${theme === "dark" ? "text-[#aed9e0]/90" : "text-[#5e6472]/80"
            }`}
        >
        ...
        <motion.div
          ...
          className={`h-1 w-48 mx-auto rounded-full mb-12 ${theme === "dark" ? "bg-[#b8f2e6]/50" : "bg-[#aed9e0]/60"
            }`}
        />
